// Initialize the map
const map = L.map("map").setView([53.759, -2.699], 14); // Set initial view to a known coordinate
...
function isOperatorSelected(operatorCode) {
  const rotalaChecked = document.getElementById("rotalaCheckbox")?.checked ?? true;
  const stagecoachChecked = document.getElementById("stagecoachCheckbox")?.checked ?? true;

  if (operatorCode === "rotala") return rotalaChecked;
...
function getFleetNumber(vehicleRef, operatorCode) {
    ...
    // Stagecoach refs come through like SCCU-12345
    if (operatorCode === "stagecoach") {
        return cleanRef.replace(/^SCCU-/i, "");
    }
	
    if (operatorCode === "rotala") {
        return cleanRef.replace(/^PBLT-/i, "");
    }	
...
async function fetchVehicleData() {
  try {
    const feedUrls = [
      "https://global.ross4122-ff0.workers.dev/operator/PBLT",
      "https://global.ross4122-ff0.workers.dev/operator/SCCU"
    ];
